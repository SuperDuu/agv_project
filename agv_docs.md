...
### Cập nhật VL53L5CX - Tăng khoảng đo
- **Tối ưu khoảng đo xa hơn**: Đã giảm tần số đo từ `10Hz` xuống `5Hz` và tăng thời gian tích phân bằng `myImager.setIntegrationTime(50);` ngay sau `setResolution(8 * 8)`. Cách này giúp sensor có nhiều thời gian thu nhận tín hiệu hơn, thường cải thiện khả năng đọc ở khoảng cách xa hơn khi môi trường đủ thoáng.
- **Lưu ý vật lý**: Nếu cảm biến vẫn chỉ đo quanh 20cm dù đã tăng integration time, nguyên nhân thường nằm ở crosstalk / che khuất cơ khí / vùng FOV bị chắn, không phải do thời gian phơi sáng.
...
- **Tối ưu quay theo góc và dò line**: Logic `Turn_IMU_Based` đã được rút gọn để bắt đầu dò line khi đạt khoảng `70%` góc đặt, rồi dừng ngay nếu mắt giữa đã chạm line. Nếu chưa thấy line sau khi chạm ngưỡng góc mục tiêu, xe tiếp tục quay tới khi bắt được line hoặc chạm timeout an toàn. Cách này giúp giảm hiện tượng đứng chờ rồi quay lố trong các góc rẽ thực tế.
- **Tối ưu xử lý ngã tư bằng state riêng**: Đã gom toàn bộ luồng `is_at_intersection` trong `main.c` vào hàm `AGV_HandleIntersectionRouting()`. Mục tiêu là giảm độ dài của `loop()`, tách rõ nhiệm vụ xử lý QR / định tuyến / quay / cập nhật heading, và hạn chế việc chồng nhiều nhánh điều kiện trong một khối duy nhất.
- **Tách state machine calibration (MODE 5) ra hàm riêng**: Phần chạy calibration trong `main.c` đã được chuyển sang `AGV_HandleMode5Calibration()`. Lý do là để tránh nhét một state machine dài vào trong vòng lặp chính, đồng thời giữ từng pha test tiến/lùi/quay IMU theo đúng thứ tự mà vẫn dễ debug và dễ mở rộng thêm các bước test mới.
- **Chuẩn hóa biến state cho calibration**: Đã đổi sang dùng biến global nội bộ `mode5_calib_state` và `mode5_calib_start_time` để giữ trạng thái calibration rõ ràng hơn, tránh vừa static trong hàm vừa khó reset khi cần thêm luồng khởi tạo lại sau này.
...
- **Ramp-up và Ramp-down chống giật cục**: Bổ sung cơ chế tăng/giảm tốc nội suy bằng vòng lặp thay vì phanh cứng `AGV_Stop` hoặc kick-start. Giúp xe mượt mà, khắc phục lỗi trượt/kẹt bánh do driver JBLD3000 sốc PWM.
- **Dò vạch liền mạch**: Logic rẽ IMU sẽ bỏ qua `AGV_Stop` và delay nếu tìm thấy vạch, cho phép xe vào thẳng trạng thái dò line ngay lập tức mà không khựng lại.
- **Bỏ qua lệnh Firebase đầu tiên**: ESP32 tự động bỏ qua lệnh Firebase lúc khởi động, chỉ kích hoạt khi `timestamp` thay đổi để tránh thực thi lệnh cũ.
