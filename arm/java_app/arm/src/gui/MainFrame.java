...
public final class MainFrame extends JFrame implements ActionListener, ChangeListener {
    ...
    private void runDualArmShowcase() {
        setGotoStatusRight("Demo: tinh IK 2 tay...", new Color(0, 90, 180));
        setGotoStatusLeft("Demo: tinh IK 2 tay...", new Color(0, 90, 180));

        new SwingWorker<DualDemoPlan, Void>() {
            @Override
            protected DualDemoPlan doInBackground() {
                return buildDualPickPlaceDemo();
            }
            ...
            protected void done() {
                try {
                    ...
                    runDualArmPlayback(plan, "Demo Ban Giao");
                } catch (Exception ex) {
                    ex.printStackTrace();
    ...
    private static class DualDemoPlan {
        ...
        final int leftGripFrameIndex;
        final int rightReleaseFrameIndex;
        final int leftReleaseFrameIndex;
        final int rightGripDelayMs;
        final int leftGripDelayMs;
        final int rightReleaseDelayMs;
        ...
        DualDemoPlan(java.util.List<double[][]> frames,
                ...
                int rightReleaseFrameIndex,
                int leftReleaseFrameIndex) {
            this(frames, rightGripFrameIndex, leftGripFrameIndex, rightReleaseFrameIndex, leftReleaseFrameIndex,
                    0, 0, 0, 0, null);
        }
        ...
        DualDemoPlan(java.util.List<double[][]> frames,
                ...
                int rightReleaseFrameIndex,
                int leftReleaseFrameIndex,
                int rightGripDelayMs,
                int leftGripDelayMs,
                int rightReleaseDelayMs,
                int leftReleaseDelayMs,
                ChairDemoScene chairScene) {
            this.frames = frames;
            this.rightGripFrameIndex = rightGripFrameIndex;
            this.leftGripFrameIndex = leftGripFrameIndex;
            this.rightReleaseFrameIndex = rightReleaseFrameIndex;
            this.leftReleaseFrameIndex = leftReleaseFrameIndex;
            this.rightGripDelayMs = rightGripDelayMs;
            this.leftGripDelayMs = leftGripDelayMs;
    ...
    private double[][] makeFlatQ1ChairFrame(double[] rightPose) {
        return new double[][] { rightPose, makeFlatQ1HoldPose(rightPose) };
    }

    private double[] makeFlatQ1HoldPose(double[] rightPose) {
        return new double[] { rightPose[0], -10, -45, 58, 18, 18 };
    }

    private void appendLinearDualArmPath(java.util.List<double[][]> frames, double[] startRight, double[] startLeft,
            double[] endRight, double[] endLeft, int steps) {
        for (int i = 1; i <= steps; i++) {
            double t = i / (double) steps;
            double[] right = new double[NUM_JOINTS];
            double[] left = new double[NUM_JOINTS];
            for (int j = 0; j < NUM_JOINTS; j++) {
                right[j] = lerp(startRight[j], endRight[j], t);
    ...
    private boolean validateFlatQ1ChairDemo(String label, java.util.List<double[][]> frames,
            ChairDemoScene scene, int samplesPerSegment, int[][] flatRanges) {
        boolean ok = true;
        ok &= validateFlatGripperPlane(label, frames, samplesPerSegment, 2.0, flatRanges);
    ...
    private boolean validateFlatGripperPlane(String label, java.util.List<double[][]> frames,
            int samplesPerSegment, double toleranceDeg, int[][] ranges) {
        double maxError = 0.0;
        int maxFrame = -1;
        int maxStep = 0;
        for (int[] range : ranges) {
            int start = Math.max(0, range[0]);
            int end = Math.min(frames.size(), range[1]);
    ...
    private int[][] inferFlatQ1DebugRanges(java.util.List<double[][]> frames) {
        java.util.List<Integer> homeFrames = new java.util.ArrayList<>();
        for (int i = 0; i < frames.size(); i++) {
            if (samePose(frames.get(i)[0], HOME_ANGLES_RIGHT, 1e-6)) {
                homeFrames.add(i);
            }
        }
        if (homeFrames.size() >= 3) {
    ...
    private boolean samePose(double[] a, double[] b, double toleranceDeg) {
        for (int i = 0; i < NUM_JOINTS; i++) {
            if (Math.abs(a[i] - b[i]) > toleranceDeg) {
                return false;
            }
        }
        return true;
    ...
    private void writeFlatQ1ChairDebugFiles(String label, java.util.List<double[][]> frames,
        ...
        try {
            ...
            int[][] flatRanges = inferFlatQ1DebugRanges(frames);
            double maxTilt = maxFlatGripperPlaneTiltDeg(frames, 8, flatRanges);
    ...
    private boolean validateDualDemoCollision(String label, java.util.List<double[][]> frames, int samplesPerSegment) {
        for (int i = 0; i < frames.size(); i++) {
            ArmPanel.CollisionResult collision = ArmPanel.diagnoseCollision(frames.get(i)[0], frames.get(i)[1]);
            if (!collision.free) {
                System.out.printf("[DEMO_COLLISION] %s frame=%d %s%n", label, i, collision);
                return false;
            }
        }
        for (int i = 0; i < frames.size() - 1; i++) {
            for (int step = 1; step < samplesPerSegment; step++) {
                ...
                if (!collision.free) {
                    System.out.printf("[DEMO_COLLISION] %s segment=%d step=%d %s%n",
                            label, i, step, collision);
                    return false;
                }
            }
        }
    ...
    private DualDemoPlan buildDualPickPlaceDemo() {
        String prefCfgRight = configComboRight.getSelectedIndex() == 0 ? "+" : "-";
        String prefCfgLeft = configComboLeft.getSelectedIndex() == 0 ? "+" : "-";
        String cacheVersion = DUAL_DEMO_CACHE_VERSION + "_r" + prefCfgRight + "_l" + prefCfgLeft;
        DualDemoPlan cachedPlan = loadDualDemoPlanCache(DUAL_DEMO_CACHE_FILE, cacheVersion);
        if (cachedPlan != null) {
            return cachedPlan;
        }
    ...
    private DualDemoPlan buildPickPlaceDemo(boolean rightDemo) {
        ...
        int gripFrameIndex = stepsPerSegment * 3;
        int releaseFrameIndex = stepsPerSegment * 8;
        DualDemoPlan plan = rightDemo
                ? new DualDemoPlan(frames, gripFrameIndex, -1, releaseFrameIndex, -1)
                : new DualDemoPlan(frames, -1, gripFrameIndex, -1, releaseFrameIndex);
    ...
    private DualDemoPlan loadDualDemoPlanCache(String fileName, String version) {
        ...
        try (java.io.BufferedReader reader = new java.io.BufferedReader(new java.io.FileReader(file))) {
            ...
            int rightGripFrameIndex = Integer.parseInt(events[0].trim());
            int leftGripFrameIndex = Integer.parseInt(events[1].trim());
            int rightReleaseFrameIndex = Integer.parseInt(events[2].trim());
            int leftReleaseFrameIndex = Integer.parseInt(events[3].trim());
            ...
            return new DualDemoPlan(
                    frames,
                    rightGripFrameIndex,
                    leftGripFrameIndex,
                    rightReleaseFrameIndex,
                    leftReleaseFrameIndex);
        } catch (Exception ex) {
    ...
    private void saveDualDemoPlanCache(DualDemoPlan plan, String fileName, String version) {
        ...
        try (java.io.PrintWriter writer = new java.io.PrintWriter(new java.io.FileWriter(file))) {
            ...
            writer.printf(java.util.Locale.US, "%d,%d,%d,%d%n",
                    plan.rightGripFrameIndex,
                    plan.leftGripFrameIndex,
                    plan.rightReleaseFrameIndex,
                    plan.leftReleaseFrameIndex);
    ...
    private void runDualArmPlayback(final DualDemoPlan plan, String statusTitle) {
        final java.util.List<double[][]> frames = plan.frames;
        if (frames.isEmpty()) {
            return;
        }
        dualDemoActive = true;
        if (plan.chairScene != null) {
            armPanel.setChairDemoScene(plan.chairScene.lowChairCenter, plan.chairScene.lowChairHeight,
                    plan.chairScene.highChairCenter, plan.chairScene.highChairHeight);
        } else {
            armPanel.clearChairDemoScene();
        }
        ...
        setDemoGripperState(false, false, "Demo 2 Tay: mo kep");
        ...
        Timer prepareTimer = new Timer(50, evt -> {
            ...
            if (arrived || waitMs[0] >= 5000) {
                ...
                final int[] eventDelayRemainingMs = { -1 };
                trajectoryTimer = new Timer(MOTION_DT_MS, e -> {
                    ...
                    boolean passThroughFrame = plan.passThroughIndices.contains(currentIndex[0]);
                    boolean reachedCurrentFrame = passThroughFrame
                            ? isDualArmNearTarget(3.0)
                            : isArmAtTarget(true) && isArmAtTarget(false);
                    if (reachedCurrentFrame) {
                        ...
                        if (eventDelayMs > 0) {
                            if (eventDelayRemainingMs[0] < 0) {
                                eventDelayRemainingMs[0] = eventDelayMs;
                                setGotoStatusRight(statusTitle + ": doi " + (eventDelayMs / 1000) + "s", Color.BLUE);
                                setGotoStatusLeft(statusTitle + ": doi " + (eventDelayMs / 1000) + "s", Color.BLUE);
                                System.out.printf("[DEMO_TRACE] wait frame=%d | delay=%dms%n",
                                        reachedFrameIndex, eventDelayMs);
                            }
                            if (eventDelayRemainingMs[0] > 0) {
                                eventDelayRemainingMs[0] -= MOTION_DT_MS;
                                updateArm();
                                return;
                            }
                        }
                        eventDelayRemainingMs[0] = -1;

                        if (rightGripNow && leftGripNow) {
                            setDemoGripperState(true, true, "Demo 2 Tay: hai tay kep");
                            if (plan.chairScene != null) {
                                armPanel.setChairDemoObjectOnHigh(false);
                            }
    ...
    private boolean isDualArmNearTarget(double toleranceDeg) {
        return isArmNearTarget(true, toleranceDeg) && isArmNearTarget(false, toleranceDeg);
    }

    private boolean isArmNearTarget(boolean isRight, double toleranceDeg) {
        double[] current = isRight ? anglesRight : anglesLeft;
        double[] target = isRight ? targetAnglesRight : targetAnglesLeft;
        for (int i = 0; i < NUM_JOINTS; i++) {
            if (Math.abs(wrappedDegDiff(target[i], current[i])) > toleranceDeg) {
                return false;
            }
        }
        return true;
