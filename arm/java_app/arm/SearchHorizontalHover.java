...
public class SearchHorizontalHover {
    public static void main(String[] args) {
        System.out.println("All valid configurations:");
        for (double q2 = -90.0; q2 <= -60.0; q2 += 10.0) {
            for (double q3 = 20.0; q3 <= 160.0; q3 += 10.0) {
                for (double sum = -50.0; sum <= -20.0; sum += 5.0) {
                    double q4 = sum - q2 - q3;
                    double[] q = { 0.0, q2, q3, q4, 0.0, 30.0 };
                    if (isWithinLimits(q)) {
                        double[] p = computeFK(q);
                        System.out.printf("q2=%.1f q3=%.1f q4=%.1f -> TCP=(%.2f, %.2f, %.2f) | pitch=%.1f\n",
                                q2, q3, q4, p[0], p[1], p[2], sum);
                    }
                }
            }
        }
