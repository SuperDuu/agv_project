...
void JointControl_Init(void) {
    ...
    for (int i = 0; i < NUM_JOINTS; i++) {
        ...
        // Initial neutral servo angle command (center of 0-270 range)
...
void JointControl_Update(float dt) {
    ...
    for (int i = 0; i < NUM_JOINTS; i++) {
        ...
        // Saturation clamping (standard servo hardware limits: 0 to 270 degrees)
        if (joint->servo_command_angle > 270.0f) joint->servo_command_angle = 270.0f;
        if (joint->servo_command_angle < 0.0f) joint->servo_command_angle = 0.0f;
        
        // 4. Update Servo Position
        Set_Servo_Angle(joint->servo_idx, (uint16_t)joint->servo_command_angle);
    }
}
